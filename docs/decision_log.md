...
## 2026-09-13 — Docker removed

Dockerfile, docker-compose.yml, docker/entrypoint.sh, .dockerignore, and the .gitattributes LF-forcing rules (which existed only for the container's benefit) all removed. Reason: Docker Desktop needs WSL2 to actually run containers on the owner's Windows machine, and neither was installed - the container was therefore built once at creation time and never actually build-tested again since. Rather than carry an unverified, unused path, removed it outright. Nothing is lost for the deployment target that matters (Streamlit Community Cloud): it never used the Dockerfile in the first place, only `requirements.txt` and the app entrypoint. `README.md` setup instructions simplified to the single (already-working) venv + `streamlit run` path.

## 2026-09-13 — Supabase connection troubleshooting note

The Postgres migration (2026-09-12 entry above) needed several rounds of troubleshooting before the connection actually worked, worth recording since it'll recur if this project's database is ever reconfigured: (1) Supabase's "Direct connection" host (`db.<ref>.supabase.co`) is IPv6-only, and the owner's network has no outbound IPv6 route at all - direct connections will never work from this machine; the fix is the Connection Pooler (Supavisor) host instead. (2) The pooler hostname includes a shard prefix (`aws-0-`, `aws-1-`, etc.) that is specific to each project and not guessable - it has to come from the dashboard's actual "Connection string" panel, not reconstructed by hand. (3) The project ultimately used is `urvknilvjymmfhvhkjjy` (region eu-west-1) - two earlier project refs (`fqawoccdpcdkahmiclnd`, whose password had been pasted in chat, and `huqahuefdedbbopemgvd`) were created and abandoned during this troubleshooting and should be deleted from the Supabase account to avoid confusion (owner action, not something this repo can do - no Supabase account-management API access exists here).
